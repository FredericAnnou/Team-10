...
X = df
y = ship_classes
kpca = PCA(n_components=5, whiten=True,svd_solver='arpack')
# 5 is good
...
train_inp = {"Cargo": X_train[mask1][:20],
            "Tanker": X_train[mask2][:20]}
mask1 = y_test=="Cargo"
mask2 = y_test=="Tanker"
...
result = qsvm.predict(test_inp["Cargo"])
result1 = qsvm.predict(test_inp["Tanker"])
pred = np.append(result, result1)
true = np.append(np.ones(len(result1)), np.zeros(len(result1)))

from sklearn.metrics import f1_score
F1 = f1_score(true, pred, average='macro')
print(f'Testing success F1: {F1}')
